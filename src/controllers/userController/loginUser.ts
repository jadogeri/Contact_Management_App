...
export const loginUser = asyncHandler(async (req : Request, res: Response)  => {
  ...
  if(user){
    ...
    if(user.isEnabled === false){      
      errorBroadcaster(res,423,"Account is locked, use forget account to access account")
    }
    ...
    if (user &&  await bcrypt.compare(password,user.password as string)) {
      ...
      if(user.failedLogins === 3){
        ...
        res.status(400).json("Account is locked beacause of too many failed login attempts. Use forget account to access account");
